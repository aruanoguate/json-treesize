...
describe('activate', () => {
  ...
  it('registers the xmlTreeSize.analyze command', () => {
  ...
  it('calls XmlTreePanel.createOrShow when a URI is provided', () => {
    const uri = { fsPath: '/test/file.xml', scheme: 'file' } as vscode.Uri;
    registeredCallback(uri);
    expect(XmlTreePanel.createOrShow).toHaveBeenCalledWith(context, uri);
  });

  it('uses activeTextEditor URI when no URI is provided', () => {
    const editorUri = { fsPath: '/editor/file.xml', scheme: 'file' } as vscode.Uri;
    (vscode.window as { activeTextEditor?: { document: { uri: vscode.Uri } } }).activeTextEditor = {
      document: { uri: editorUri },
    };

    registeredCallback(undefined);
    expect(XmlTreePanel.createOrShow).toHaveBeenCalledWith(context, editorUri);
  ...
  it('shows error message when no URI and no active editor', () => {
    (vscode.window as { activeTextEditor?: unknown }).activeTextEditor = undefined;
    registeredCallback(undefined);
    expect(vscode.window.showErrorMessage).toHaveBeenCalledWith('No XML file selected.');
    expect(XmlTreePanel.createOrShow).not.toHaveBeenCalled();
  });
});
